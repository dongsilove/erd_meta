...
DROP TABLE IF EXISTS T_CM_MTRPIP;
...
DROP TABLE IF EXISTS T_INEV_SOIL_CRSVN;
...
(
	...
	-- 도로 유형 명
	ROAD_TY_NM VARCHAR(200),
	CONSTRAINT pk_t_am_wtl_pip PRIMARY KEY (ASST_SN)
) WITHOUT OIDS;


-- 관종
CREATE TABLE T_CM_MTRPIP
(
	-- 관종 코드
	MTRPIP_CD VARCHAR(20) NOT NULL,
	-- 재질
	MTRQLT VARCHAR(100),
	-- 내면 피복 재질
	INNSF_CLTH_MTRQLT VARCHAR(100),
	-- 외면 피복 재질
	OUTSF_CLTH_MTRQLT VARCHAR(100),
	CONSTRAINT pk_t_cm_mtrpip PRIMARY KEY (MTRPIP_CD)
) WITHOUT OIDS;
...
(
	-- 평가 관종 코드
	EVL_MTRPIP_CD VARCHAR(20) NOT NULL,
	-- 평가 등급 코드
	EVL_GRAD_CD VARCHAR(5) NOT NULL,
	-- 범위 시작 값
	RANG_BEGIN_VAL NUMERIC,
	-- 범위 종료 값
	RANG_END_VAL NUMERIC,
	...
	CONSTRAINT pk_t_inev_grad PRIMARY KEY (EVL_MTRPIP_CD, EVL_GRAD_CD)
) WITHOUT OIDS;
...
-- 간접평가 토양부식성
CREATE TABLE T_INEV_SOIL_CRSVN
(
	-- 토양 소블록 명
	SOIL_SBLK_NM VARCHAR(200) NOT NULL,
	-- 토양 부식성
	SOIL_CRSVN NUMERIC,
	CONSTRAINT pk_t_inev_soil_crsvn PRIMARY KEY (SOIL_SBLK_NM)
) WITHOUT OIDS;
...
(
	...
	-- 가중치 값
	WGHT_VAL NUMERIC,
	CONSTRAINT pk_t_inev_wght PRIMARY KEY (EVL_ITM_CD, EVL_MTRPIP_CD)
) WITHOUT OIDS;
...
(
	...
	EVL_MTRPIP_CD VARCHAR(20),
	-- 평가 등급 코드
	EVL_GRAD_CD VARCHAR(5),
	-- 평가 결과 점수
	EVL_RST_SCOR NUMERIC,
	-- 누수 민원 수
	WTRLK_CVPL_CO NUMERIC,
	-- 이형 관종 수
	HETERO_MTRPIP_CO NUMERIC,
	-- 파손 수
	BRKDN_CO NUMERIC,
	-- 급수 가지 수
	...
	LYNG_YYNU NUMERIC,
	-- 수질 부식성
	QLTWTR_CRSVN NUMERIC,
	-- 토양 소블록 명
	SOIL_SBLK_NM VARCHAR(200),
	-- 토양 부식성
	SOIL_CRSVN NUMERIC,
	-- 전기 방식 값
	ELTR_MENS_VAL NUMERIC,
...
COMMENT ON COLUMN T_AM_WTL_PIP.MTRPIP_CD IS '관종 코드';
COMMENT ON COLUMN T_AM_WTL_PIP.PIP_DMTR IS '관경';
COMMENT ON COLUMN T_AM_WTL_PIP.PIP_OUTDMTR IS '관 외경';
COMMENT ON COLUMN T_AM_WTL_PIP.PIP_THKNS IS '관 두께';
COMMENT ON COLUMN T_AM_WTL_PIP.PIP_LT IS '관 길이';
...
COMMENT ON COLUMN T_AM_WTL_PIP.ROAD_TY_NM IS '도로 유형 명';
COMMENT ON TABLE T_CM_MTRPIP IS '관종';
COMMENT ON COLUMN T_CM_MTRPIP.MTRPIP_CD IS '관종 코드';
COMMENT ON COLUMN T_CM_MTRPIP.MTRQLT IS '재질';
COMMENT ON COLUMN T_CM_MTRPIP.INNSF_CLTH_MTRQLT IS '내면 피복 재질';
COMMENT ON COLUMN T_CM_MTRPIP.OUTSF_CLTH_MTRQLT IS '외면 피복 재질';
...
COMMENT ON COLUMN T_INEV_CND.EVL_MTRPIP_CD IS '평가 관종 코드';
...
COMMENT ON COLUMN T_INEV_CND.CND_VAL IS '조건 값';
COMMENT ON TABLE T_INEV_GRAD IS '간접평가상태등급 : 간접평가상태등급';
COMMENT ON COLUMN T_INEV_GRAD.EVL_MTRPIP_CD IS '평가 관종 코드';
COMMENT ON COLUMN T_INEV_GRAD.EVL_GRAD_CD IS '평가 등급 코드';
...
COMMENT ON TABLE T_INEV_SOIL_CRSVN IS '간접평가 토양부식성';
COMMENT ON COLUMN T_INEV_SOIL_CRSVN.SOIL_SBLK_NM IS '토양 소블록 명';
COMMENT ON COLUMN T_INEV_SOIL_CRSVN.SOIL_CRSVN IS '토양 부식성';
COMMENT ON TABLE T_INEV_WGHT IS '간접평가 가중치';
COMMENT ON COLUMN T_INEV_WGHT.EVL_ITM_CD IS '평가 항목 코드';
COMMENT ON COLUMN T_INEV_WGHT.EVL_MTRPIP_CD IS '평가 관종 코드';
...
COMMENT ON COLUMN T_INEV_WTL_PIP.SBLK_NM IS '소블록 명';
COMMENT ON COLUMN T_INEV_WTL_PIP.DGN_SECT_NM IS '진단 구간 명';
COMMENT ON COLUMN T_INEV_WTL_PIP.MTRPIP_CD IS '관종 코드';
COMMENT ON COLUMN T_INEV_WTL_PIP.PIP_LT IS '관 길이';
COMMENT ON COLUMN T_INEV_WTL_PIP.EVL_MTRPIP_CD IS '평가 관종 코드';
COMMENT ON COLUMN T_INEV_WTL_PIP.EVL_GRAD_CD IS '평가 등급 코드';
COMMENT ON COLUMN T_INEV_WTL_PIP.EVL_RST_SCOR IS '평가 결과 점수';
COMMENT ON COLUMN T_INEV_WTL_PIP.WTRLK_CVPL_CO IS '누수 민원 수';
COMMENT ON COLUMN T_INEV_WTL_PIP.HETERO_MTRPIP_CO IS '이형 관종 수';
COMMENT ON COLUMN T_INEV_WTL_PIP.BRKDN_CO IS '파손 수';
COMMENT ON COLUMN T_INEV_WTL_PIP.WTRSPL_BRANCH_CO IS '급수 가지 수';
COMMENT ON COLUMN T_INEV_WTL_PIP.VALE_CO IS '밸브 수';
COMMENT ON COLUMN T_INEV_WTL_PIP.INSTALL_YMD IS '설치일';
COMMENT ON COLUMN T_INEV_WTL_PIP.PIP_DMTR IS '관경';
COMMENT ON COLUMN T_INEV_WTL_PIP.LYNG_YYNU IS '매설 년수';
COMMENT ON COLUMN T_INEV_WTL_PIP.QLTWTR_CRSVN IS '수질 부식성';
COMMENT ON COLUMN T_INEV_WTL_PIP.SOIL_SBLK_NM IS '토양 소블록 명';
COMMENT ON COLUMN T_INEV_WTL_PIP.SOIL_CRSVN IS '토양 부식성';
COMMENT ON COLUMN T_INEV_WTL_PIP.ELTR_MENS_VAL IS '전기 방식 값';
COMMENT ON COLUMN T_INEV_WTL_PIP.SOIL_KND IS '토양 종류';
COMMENT ON COLUMN T_INEV_WTL_PIP.LYNG_DP IS '매설 깊이';
COMMENT ON COLUMN T_INEV_WTL_PIP.ROAD_TY_NM IS '도로 유형 명';
COMMENT ON COLUMN T_INEV_WTL_PIP.MXMM_PRSRWTR IS '최대 수압';
COMMENT ON COLUMN T_INEV_WTL_PIP.CNNCT_DN IS '이음 밀도';
COMMENT ON COLUMN T_INEV_WTL_PIP.SECT_CNNCT_DN IS '구간 이음 밀도';
COMMENT ON COLUMN T_INEV_WTL_PIP.SECT_PIP_LT IS '구간 관 길이';
